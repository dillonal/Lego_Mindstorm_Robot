...
def move_bot(distance, direction): #distance in inches; true for direction = forward, flase for direction = backwards
    ...
    if (direction == "forward"):
        ...
        for i in range (0, distance - 1):
            tank_pair.on_for_rotations(const_speed,const_speed,0.119583)
            if (abs(angle - tempangle) < 2):
                while (angle - tempangle != 0):
...
def turn_bot(direction):
    if (direction == "right"):
        ...
        angle = gy.angle
        tempangle = gy.angle
        while (abs(angle - tempangle) < 83):        # check angle for turning on correct surface
            tank_pair.on_for_seconds(-10,10,0.05)
            angle = gy.angle
    elif (direction == "left"):
        gy.MODE_GYRO_RATE
...
def lift_fork(direction):
    if (direction == "up"):
        lift.on_for_seconds(speed = 10, seconds = 2)
    if (direction == "down"):
        lift.on_for_seconds(speed = -10, seconds = 2)
